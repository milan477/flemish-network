...
## UX Remediation — Completed 2026-05-10

All seven phases of `docs/plans/UX_REMEDIATION_2026-05-10.md` shipped:

- **Phase 1 — Auth signOut storm + legacy routes.** `src/lib/auth.tsx` no longer broadcasts a global SIGNED_OUT on transient `loadStaffUser` failure; cached profile is retained with a `console.warn`. A single-flight ref keyed by `user.id` collapses concurrent `INITIAL_SESSION`/`TOKEN_REFRESHED`/focus calls into one `activate_staff_user_session` RPC per session change. The auth-failure cleanup path now uses `signOut({ scope: 'local' })`. `AuthProvider` value memo is fully stable (callbacks via `useCallback`, derived `canEdit`/`isAdmin` hoisted). `src/App.tsx` registers explicit `<Navigate>` aliases for `/admin/overview`, `/admin/discovered`, `/admin/agents`, and `/contacts/new` ahead of the `<RequireAuth>` boundary.
- **Phase 2 — `suggest-people` 401 + `'rest'` banner.** `supabase/functions/suggest-people/index.ts` `callUntypedRpc` now invokes through the client so PostgREST's `this.rest` is bound (matches the fixed pattern in `search-people`). All shared edge-function imports pin `npm:@supabase/supabase-js@2.57.4` and every active function was redeployed in one pass so warm Deno workers cold-start and re-fetch JWKS (eliminates the ES256 kid drift).
- **Phase 3 — Autofill latency.** New RPC `search_people_autofill(q, lim)` (`20260510120000_autofill_rpc.sql`) runs over the trigram-GIN-indexed `*_search_documents.name_normalized` columns. `UnifiedSearchBar.tsx` now calls the RPC once per keystroke instead of two un-indexed ILIKE chains, attaches an `AbortController`, debounces 250 ms, requires `q.length >= 2`, drops the unused `locations(*)` join, and memoises the last 20 queries in an in-memory LRU.
- **Phase 4 — PersonProfile auto-verify + URL validation.** Save no longer writes `last_verified_at` — verification stays owned by the verify pipeline. `ensureProtocol` replaced by a strict `normalizeUrl` validator that rejects garbage strings on `linkedin_url`, `twitter_url`, `website_url` and prevents the same-origin-relative `<a>` trap.
- **Phase 5 — Verification UI filter + realtime callback.** `DiscoveredContactsPanel.tsx` queue load drops the legacy `status='pending'` filter and keys on `verification_status IN ('queued','verifying','verified','failed')`; per-row NULL fallback is now `'queued'` (was `'verified'`, which silently bypassed the gate). `handleRejectAll`/`handleRejectAllOrganizations` filter to verified rows only, require confirmation + reason, and write `reject_reason` before delete. Realtime `.subscribe()` callback logs SUBSCRIBED / CHANNEL_ERROR / TIMED_OUT.
- **Phase 6 — `agent-scheduler` verification dispatch.** `enqueueVerificationBatch` falls back to `SUPABASE_SERVICE_ROLE_KEY` when no caller auth is forwarded; dispatch failures persist to `agent_runs.error_message`/`error_kind` and increment a new `verification_attempts` column. At ≥3 attempts the row escalates to `verification_status='failed'` (`20260510130000_verification_failure_state.sql` adds the column and extends the CHECK constraint). The Verification UI surfaces failed rows with a red banner.
- **Phase 7 — P2/P3 cleanup.** Dashboard no longer forces `view=list` on every successful query. `UnifiedSearchBar` clears its debounce on submit/clear/select to prevent the post-Enter reopen. Manual-entry validator on `AddContactPanel` requires `last_name`. `AccessManagementPanel` disables the self-row Remove button with a `title` hint. `AgentDashboard.formatDuration` renders multi-day spans as `1d 11h 46m` instead of `2146m 6s`. `SystemHealthPanel` "Est. cost" reads "—" when both Gemini and Tavily counters are zero. New unique partial indexes on `collection_members` (`20260510140000_collection_members_uq.sql`) provide race-safe duplicate prevention. React Router future flags (`v7_startTransition`, `v7_relativeSplatPath`) opted in at `src/main.tsx`.

`npm run typecheck`, `npm test` (132/132), and `npm run build` all green at completion. Migrations `20260510120000_autofill_rpc.sql`, `20260510130000_verification_failure_state.sql`, and `20260510140000_collection_members_uq.sql` applied to project `ofzuhajxwxggybkuzefq` via `supabase db push --linked`. All active edge functions redeployed in the same session. Live in-browser verification of Phase 7 follow-ups (`/organizations/:id` reflow, race-condition test for duplicate `collection_members` insert, `/login` flows) deferred to manual QA — the Chrome MCP extension blocks input on credential pages, and the prior session's auth bundle is now the gating fix for any human-driven re-test.
...
## Operations Fix — Completed 2026-09-14

Goal: make the platform usable by Flemish government staff for a feedback round, which needs the agents to run unattended and the database to contain only real, reviewed records.
