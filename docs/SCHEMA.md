...
| `scripts/seed_phase3_search_dataset.ts` (`npm run seed:phase3`) | Destructive synthetic Phase 3 reset/reseed. Requires `VITE_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and `PHASE3_RESET_CONFIRM=ofzuhajxwxggybkuzefq`. Deletes approved people/organizations and dependent search, embedding, sector/Flemish/location-link, and collection membership rows; preserves staff users, discovery history, surfaces/lenses/seed domains, sectors, locations, and Flemish catalog rows; seeds 160 fake people and 75 fake organizations. Last run 2026-05-07; on 2026-09-14 every synthetic `people`/`organizations` row (and dependents) was purged from the linked project so the approved tables only ever hold reviewed real records again. Do not run this against the linked project while staff are evaluating it. |
| `20260914210000_agent_scheduler_tick_timeout.sql` | Re-schedules the `agent-scheduler-tick` pg_cron job with `timeout_milliseconds := 30000` (the 5 s default logged spurious timeouts once the tick started running zombie cleanup and hourly housekeeping). Same vault-backed URL/key lookup as before. Applied to linked project `ofzuhajxwxggybkuzefq` on 2026-09-14. |
