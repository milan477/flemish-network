...
3. Fetches pages, stores `discovery_pages`, classifies pages, extracts candidates, and stores evidence. The run budgets itself against the free-plan 150 s wall clock: seeding and page processing stop at `DEADLINE_MS = 80 s`, and no new page starts with less than 30 s left (`PAGE_START_BUDGET_MS`), so the wrap-up writes (run results, `resolve_discovery_query_attempts`, arm stats) always finish before the worker is killed. A run killed mid-wrap-up never records results and only surfaces as a zombie.
...
3. Discovered-record path uses web search + Gemini structured output. The prompt requires `network_scope` to be `null` when no clear residence/tie signal is present (no guessing). `contradiction=true` triggers a hard delete; ambiguity does not. Nullable fields in `VERIFICATION_SCHEMA` (`supabase/functions/_shared/discoveredVerification.ts`) must be written as `{ type: "string", nullable: true }`; Gemini rejects JSON-Schema unions like `type: ["string", "null"]` with HTTP 400, which made every verify-before-promote call fail from May until 2026-09-14. `discoveredVerification_test.ts` guards the schema shape.
...
- Verification dispatch falls back to `SUPABASE_SERVICE_ROLE_KEY` for the `agent-verify` invocation when the caller did not forward an `Authorization` header (e.g. anonymous housekeeping). Dispatch failures are persisted to `agent_runs.error_message`/`error_kind` and increment `discovered_contacts.verification_attempts` / `discovered_organizations.verification_attempts`. At ≥3 attempts the row escalates to `verification_status='failed'` and is surfaced in `/admin/verification` rather than silently re-queued.
- **Internal service auth (fixed 2026-09-14).** `agent-discovery`, `agent-verify`, `agent-discovery-reflect`, and `generate-embeddings` authenticate with `requireStaffOrServiceRole` from `_shared/auth.ts`. A request whose bearer (or `apikey`) is byte-equal to `SUPABASE_SERVICE_ROLE_KEY` or to one of `SUPABASE_SECRET_KEYS` is an internal call and runs as a synthetic `editor`; anything else goes through the normal `auth.getUser` staff check. Before this, the pg_cron path dispatched with the service key, `auth.getUser` answered `invalid claim: missing sub claim`, and every scheduled run died as a `Zombie: no heartbeat` (`db_timeout`) while manual runs (user JWT forwarded) kept working. Any new function that the scheduler invokes must use `requireStaffOrServiceRole`, never `requireStaffRole`.
- **Dispatch is observed, not fire-and-forget.** `dispatchAgentFunction` in `agent-scheduler` sends the downstream request, returns immediately to the caller, and keeps the response promise alive with `EdgeRuntime.waitUntil`. A non-2xx answer (401 `auth_failed`, 429 `quota_exhausted`, 5xx `agent_failure`) or a network error is written to the run row (`status='failed'`, `error_message`, `error_kind`) only while the run is still `pending`/`running`, so the agent function's own failure bookkeeping wins once it has accepted the request. The `embeddings_drain` kick throws on a non-2xx response so `agent_schedules.last_error` records it.
...
`tick` is service-role-only: the Authorization bearer must be byte-equal to the key edge functions receive as `SUPABASE_SERVICE_ROLE_KEY` (today the project's `sb_secret_…` default secret key) or to one of `SUPABASE_SECRET_KEYS`. A JWT that merely claims `role=service_role` is rejected because gateway JWT verification is disabled for this function. It is invoked every 5 minutes by the `agent-scheduler-tick` pg_cron job with a 30 s `timeout_milliseconds`. The job posts to `<project_url>/functions/v1/agent-scheduler` with `{ "action": "tick", "source": "pg_cron" }`, reading `project_url` and `service_role_key` from `vault.decrypted_secrets`; the vault `service_role_key` therefore has to hold that same secret key (rotated on 2026-09-14 from the legacy JWT).

Each tick first runs `markZombieRuns` (a `running` row with a heartbeat older than 2 minutes becomes `failed` "Zombie: no heartbeat"; a `pending` row older than 10 minutes becomes `failed` "Zombie: never started") so a run whose function died cannot block later scheduled runs with `skipped_busy`, then dispatches due jobs, and on the first tick of every UTC hour (`minute < 5`) runs the full `housekeeping` routine with the tick's own credentials so verification enqueue, reflection, pivot rebuilds, arm stats, and domain reputation keep moving when nobody opens the admin UI. The response is `{ status: "ok", tick: { outcomes, zombies_marked_failed, housekeeping } }`.
