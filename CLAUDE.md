...
- Flemish connections are now normalized for people: migration `20260330000000_normalize_flemish_connections.sql` adds `flemish_connections` plus `person_flemish_connections`, keeps `people.flemish_connection` as a canonical comma-joined compatibility field, and backfills existing rows through SQL extraction functions.
...
- Admin Flemish-connection charts now count discrete tags instead of raw comma strings, and the `discover_connections()` alumni pass now reads `person_flemish_connections` instead of splitting `people.flemish_connection`.
