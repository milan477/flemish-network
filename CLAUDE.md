...
- Follow-up on 2026-04-03: the project is now on Supabase's newer asymmetric auth tokens, and the platform's legacy built-in Edge Function JWT gate was rejecting real staff access tokens with platform-level `401 Invalid JWT` responses before any function code ran. `supabase/config.toml` now sets `verify_jwt = false` for every deployed function because each handler already enforces auth internally via `requireStaffRole()`. Live smoke tests with a real magic-link staff session confirmed `ai-agent`, `search-people`, `discover-contacts`, `update-profile`, and `agent-scheduler` now execute again under authenticated access, and `agent-connections` now runs one relationship type per RPC instead of one large all-types statement to stay under the database statement timeout on first-run backfills.
...
- **Edge Functions self-authenticate now:** because the project uses Supabase's newer asymmetric auth tokens, the platform's legacy built-in function JWT check must stay disabled. `supabase/config.toml` sets `verify_jwt = false` for every function, and each handler is responsible for auth via `requireStaffRole()` (or equivalent in-function checks) instead of relying on the outer gateway check.
