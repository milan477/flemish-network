...
import { exportOrganizationsToCsv, exportOrganizationsToExcel } from '../lib/exportService';
import { notifyError } from '../lib/toast';

interface OrganizationExportMenuProps {
...
export default function OrganizationExportMenu({
  organizations,
  filename,
  buttonClassName = DEFAULT_BUTTON_CLASS,
}: OrganizationExportMenuProps) {
  ...
  const handleExport = async (format: ExportFormat) => {
    ...
    try {
      if (format === 'xlsx') {
        await exportOrganizationsToExcel(organizations, filename);
      } else {
        await exportOrganizationsToCsv(organizations, filename);
      }
      setOpen(false);
    } catch (err) {
