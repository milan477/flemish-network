...
- [x] Add staff-only authentication, account management, and role gates for the application shell.
  ...
  Update (2026-04-03): Follow-up hardening for the same rollout. The linked project now uses Supabase's newer asymmetric auth tokens, and the platform's legacy built-in Edge Function JWT gate was rejecting real staff sessions with platform-level `401 Invalid JWT` responses before handler code ran. Added `supabase/config.toml` with `verify_jwt = false` for every function because the codebase already authenticates inside each handler with `requireStaffRole()`, then redeployed all functions and confirmed authenticated `ai-agent`, `search-people`, `discover-contacts`, `update-profile`, and `agent-scheduler` calls succeed again with a real magic-link staff session.
...
- [x] Expand connections conservatively and keep soft affinity separate from hard graph edges.
  ...
  Update (2026-04-03): `agent-connections` now runs `discover_connections()` one relationship type at a time with heartbeats between calls instead of one all-types RPC. This avoids the first-run statement timeout that showed up when backfilling a large batch of hard edges on production data.
